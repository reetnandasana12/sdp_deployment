...
function HotelData() {
  ...
  const processDataForChart = (hotels, option) => {
    const data = {};
            ...
            tooltip: {
              callbacks: {
                label: function (context) {
                  return `hotels Added: ${context.parsed.y}`;
                },
              },
            },
          },
  ...
  useEffect(() => {
    if (hotels.length > 0) {
      const data = processDataForChart(hotels, selectedOption);
      drawOrUpdateChart('carChart', data);
    }
  }, [hotels, chartInstance, selectedOption]);
